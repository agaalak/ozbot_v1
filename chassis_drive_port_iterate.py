...
# Initialize Roboclaw
roboclaw_port = find_roboclaw()
if roboclaw_port is None:
    print('Unable to find Roboclaw')
    exit(1)
else:
    print(f'Found Roboclaw at {roboclaw_port}')
...
def main():
    ...
    # Try to open all video devices
    for i in range(32):
        cap = cv2.VideoCapture(f'/dev/video{i}')
        if cap is None or not cap.isOpened():
            print(f'Unable to open video device /dev/video{i}')
        else:
            print(f'Successfully opened video device /dev/video{i}')
            break

    # If no device is opened successfully, exit the script
    if cap is None or not cap.isOpened():
